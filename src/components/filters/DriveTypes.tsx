...
const FILTER_NAME = "DriveTypes";

export default function DriveTypes() {
  const { drives, checked, isOpen, toggle, anyValue, clearFilter } =
    useDriveTypes();
    ...
    >
      <Collapsible.Trigger className="btn f-trigger" asChild>
        <div className="f-trigger-inner">
          <span className="flex-fill">Tracción</span>
          {anyValue && (
...
const useDriveTypes = () => {
  ...
  return { drives, checked, isOpen, toggle, anyValue, clearFilter };
};
