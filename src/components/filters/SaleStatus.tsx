...
const FILTER_NAME = "SaleStatuses";

export default function SaleStatus() {
  const { statuses, checked, isOpen, toggle, anyValue, clearFilter } =
    useDriveTypes();
    ...
    >
      <Collapsible.Trigger className="btn f-trigger" asChild>
        <div className="f-trigger-inner">
          <span className="flex-fill">Estatus de la venta</span>
          {anyValue && (
...
const useDriveTypes = () => {
  ...
  return { statuses, checked, isOpen, toggle, anyValue, clearFilter };
};
